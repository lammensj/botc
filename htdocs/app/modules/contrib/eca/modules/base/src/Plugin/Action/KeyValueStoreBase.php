...
abstract class KeyValueStoreBase extends ConfigurableActionBase {
   ...
   * {@inheritdoc}
   */
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition): KeyValueStoreBase {
    /**
     * @var \Drupal\eca_base\Plugin\Action\KeyValueStoreBase $instance
     */
   ...
   */
  abstract protected function store(string $collection);
   ...
   * @throws \Exception
   */
  protected function doStore(string $collection, bool $ifNotExists, string $key, $value): void {
    if ($ifNotExists) {
  ...
  public function execute(): void {
    $collection = $this->tokenServices->replaceClear($this->configuration['collection']);
    $key = $this->tokenServices->replaceClear($this->configuration['key']);
    if ($this->writeMode()) {
